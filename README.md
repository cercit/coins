...
A personal numismatic collection — 674 coins from 102 countries and 121 issuing
authorities — browsable by world map or by flag. Pick a country, and the drawer opens:
every coin photographed front and back, background removed, with denomination, year and a
short write-up on what's distinctive about that country's coinage.

Static site, no build step. `index.html` + `assets/app.js` render everything client-side
from `data/collection.json`.
...
The home page opens with a world map of the collection. Each country is filled
copper-to-gold by how many coins it holds — brighter means more — on a dark cabinet
background, while uncollected countries stay quiet. Selecting a country opens the same
Country → Issuer → coin drawer as the flag grid, so the map is an entry point, not a
separate view. Historical authorities (British India, the princely states) live beneath
their modern country rather than as their own map shapes. A World / continent selector
zooms the frame so smaller countries are reachable; the search box and flag grid below
stay the reliable way to reach every country, including the handful too small to draw as
their own shape.

The map is a single local file, `assets/world-map.svg`, generated by
`npm run build-world-map` from public-domain Natural Earth geometry whose India polygon has
been completed to India's official borders (Aksai Chin, Pakistan-occupied Kashmir,
Arunachal Pradesh, and the Andaman & Nicobar Islands and Lakshadweep). It pulls in no map SDK, API key or third-party runtime. If the map is
...
assets/app.js          routing + rendering, guilloché generator, world map
assets/map-utils.js    pure map helpers (country index, colour buckets, aria labels)
assets/world-map.svg   generated — Robinson world map, one <path> per country (data-iso)
...
scripts/build-world-map.js  builds assets/world-map.svg from India-complete Natural Earth data
